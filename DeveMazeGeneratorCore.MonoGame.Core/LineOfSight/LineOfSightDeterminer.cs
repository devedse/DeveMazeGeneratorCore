...
namespace DeveMazeGeneratorMonoGame.LineOfSight
{
    public class LineOfSightDeterminer
    {
        ...
        private List<MazePointPos> path;
        private int current = 0;

        public LineOfSightDeterminer(InnerMap innerMap, List<MazePointPos> path)
        {
        ...
        public LineOfSightObject GetNextLosObject()
        {
            ...
            {
                List<MazePointPos> adjacentPoints = new List<MazePointPos>();
                ...
                {
                    adjacentPoints.Add(new MazePointPos(x - 2, y));
                }
                ...
                {
                    adjacentPoints.Add(new MazePointPos(x, y - 2));
                }
        ...
        public List<MazePointPos> GetAdjacentPoints(MazePointPos curMazePoint, int curPathStep)
        {
        ...
        private Boolean HasLosSmart(MazePointPos start, MazePointPos end)
        {
            if (start.X < end.X)
            {
        ...
        private Boolean HasLos(MazePointPos start, MazePointPos end)
        {
