...
{
    public class AlgorithmBacktrack5_UnsafeV2 : IAlgorithm<Maze>
    {
            ...
            where TAction : struct, IProgressAction
        {
            ...
            // Special handling to allow inlining of the indexer
            if (innerMap is BitArreintjeFastInnerMapUnsafeV2 unsafeMap)
            {
                // We pass the concrete type so the JIT can inline the unsafe indexer
                return GoGenerateInternal(unsafeMap, random, pixelChangedCallback);
            }
            else
            {
                // Fallback for other maps (will use virtual calls and likely be slower because of that, plus basic map speed)
                return GoGenerateInternal(innerMap, random, pixelChangedCallback);
            }
        }

        // This method takes the CONCRETE type BitArreintjeFastInnerMapUnsafeV2
        private Maze GoGenerateInternal<TAction>(BitArreintjeFastInnerMapUnsafeV2 map, IRandom random, TAction pixelChangedCallback)
            where TAction : struct, IProgressAction
        {
        ...
        // Generic fallback
        private Maze GoGenerateInternal<M, TAction>(M map, IRandom random, TAction pixelChangedCallback)
             where M : InnerMap
             where TAction : struct, IProgressAction
        {
            long totSteps = (map.Width - 1L) / 2L * ((map.Height - 1L) / 2L);
