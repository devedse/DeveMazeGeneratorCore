...
namespace DeveMazeGenerator.Generators
{
    public abstract class AlgorithmWithPath : Algorithm
    {
        ...
        public InnerMap GenerateWithPath<M, R>(int width, int height, Action<int, int, long, long> pixelChangedCallback) where M : InnerMap where R : IRandom
        {
        ...
        /// <param name="pixelChangedCallback">When a pixel is changed you can define a callback here to for example draw the maze while its being generated, add null if you don't want this. Last 2 longs are for the current step and the total steps (can be used to calculate how far the maze is done being generated)</param>
        public InnerMap GenerateWithPath<M, R>(int width, int height, int seed, Action<int, int, long, long> pixelChangedCallback) where M : InnerMap where R : IRandom
        {
        ...
        public abstract InnerMap GoGenerateWithPath<M>(IInnerMapFactory<M> mapFactory, IRandomFactory randomFactory, Action<int, int, long, long> pixelChangedCallback) where M : InnerMap;
    }
}
